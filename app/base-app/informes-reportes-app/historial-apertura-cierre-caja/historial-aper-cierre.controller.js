(function () {
    ...
    function HistorialApeCierreCajaCtrl($scope, historialApeCierreCajaService, ValidationService,  localStorageService, $filter, $http,  utilServices, appConstant, appGenericConstant) {
        ...
        gestionHistorial.onConsultarListado = function () {
            var totalReporteApertura = 0;
            var totalReporteCierreReal = 0;
            var totalReporteCierreSistema = 0;
                ...
                historialApeCierreCajaService.consultarBetweenFechas(toDate(gestionHistorial.fechaApertura), toDate(gestionHistorial.fechaCierre)).then(function (data) {
                    ...
                    angular.forEach(data, function (value) {
                        ...
                        var datos = {
                            ...
                            nombreSupervisorApertura: value.nombreSupervisorApertura,
                            nombreSupervisorCierre: value.nombreSupervisorCierre,
                            totalApertura: $filter('currency')(value.totalApertura),
                            totalRealCierre: $filter('currency')(value.totalRealCierre),
                            totalSistemaCierre: $filter('currency')(value.totalSistemaCierre)
                        };
                        totalReporteApertura = totalReporteApertura + value.totalApertura;
                        totalReporteCierreReal = totalReporteCierreReal + value.totalRealCierre;
                        totalReporteCierreSistema = totalReporteCierreSistema + value.totalSistemaCierre;
                        gestionHistorial.listaGeneral.push(datos);
                    });
                    ...
                    var datosTotal = {
                        ...
                        fechaCierreSt: '',
                        nombreCajero: '',
                        nombreSupervisorApertura: '',
                        nombreSupervisorCierre: 'TOTALES',
                        totalApertura: $filter('currency')(totalReporteApertura),
                        totalRealCierre: $filter('currency')(totalReporteCierreReal),
                        totalSistemaCierre: $filter('currency')(totalReporteCierreSistema)
                    };
                    gestionHistorial.listaGeneral.push(datosTotal);
        ...
        gestionHistorial.onObtenerLista = function (fechaApertura, fechaCierre) {
            appConstant.MSG_LOADING(appGenericConstant.CARGANDO);
            appConstant.CARGANDO();
            gestionHistorial.listaGeneral = [];
            historialApeCierreCajaService.consultarBetweenFechas(toDate(fechaApertura), toDate(fechaCierre)).then(function (data) {
