(function () {
    'use strict';
    angular.module('mytodoApp').controller('AsistenciaPeriodoCtrl', AsistenciaPeriodoCtrl);
    AsistenciaPeriodoCtrl.$inject = ['$scope', 'asistenciaServices', 'cambioHorarioServices', 'localStorageService', '$filter', 'utilServices', 'appConstant', '$window', 'appGenericConstant', '$http', 'confiEducacionContinuadaServices'];
    function AsistenciaPeriodoCtrl($scope, asistenciaServices, cambioHorarioServices, localStorageService, $filter, utilServices, appConstant, $window, appGenericConstant, $http, confiEducacionContinuadaServices) {
        ...
        gestionAsistencia.listadoPeriodos = [];
        gestionAsistencia.periodoSelect = {};
            ...
            asistenciaServices.getProgramasByNivelFormacion(idNivelFormacion).then(function (data) {
                ...
                asistenciaServices.getPeriodosAcademicosAll().then(function (data) {
                    angular.forEach(data, function (value, key) {
                        ...
                        gestionAsistencia.listadoPeriodos.push(periodo);
                    });
                });
        ...
        gestionAsistencia.onFiltrarAsistencia = function () {
            ...
            asistenciaServices.getListadoAsistenciaByProgramaCertificado(gestionAsistencia.programa.id, gestionAsistencia.periodoSelect.id).then(function (data) {
        ...
        gestionAsistencia.onFiltrarAsistenciaDelDia = function () {
            ...
            appConstant.CARGANDO();
            gestionAsistencia.disabledCodigo = false;
            asistenciaServices.getListadoAsistenciaByProgramaDelDia(gestionAsistencia.programa.id, gestionAsistencia.periodoSelect.id).then(function (data) {
                gestionAsistencia.listAsistencia = data;
                appConstant.CERRAR_SWAL();
            }).catch(function (e) {
                appConstant.CERRAR_SWAL();
        ...
        $('#codigo').keypress(function (e) {
            if (e.which === 13) {
                ...
                asistenciaServices.getListadoAsistenciaByProgramaAsistenciaDelDia(gestionAsistencia.programa.id, gestionAsistencia.periodoSelect.id, gestionAsistencia.codigo).then(function (data) {
                    if (data) {
                    ...
                    asistenciaServices.getListadoAsistenciaByProgramaDisponible(gestionAsistencia.programa.id, gestionAsistencia.periodoSelect.id, gestionAsistencia.codigo).then(function (data) {
        ...
        gestionAsistencia.onBuscarConfiguracionEduCon = function (item) {
            ...
            appConstant.CARGANDO();
            confiEducacionContinuadaServices.buscarConfiguracionByProgramaAndPeriodoAcademico(gestionAsistencia.programa.id, gestionAsistencia.periodoSelect.id).then(function (data) {
        ...
        gestionAsistencia.onGenerarReporte = function (info, num) {
            ...
            $http.post(urlRequest, jsonString, headers).then(function (data) {
                if (data.status === 200) {
                    gestionAsistencia.item.push(data.data.message);
                    gestionAsistencia.download(gestionAsistencia.item[0]);
                } else {
