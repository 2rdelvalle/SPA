(function () {
    'use strict';
    angular.module('mytodoApp').controller('liquidarConceptosCtrl', liquidarConceptosCtrl);

    liquidarConceptosCtrl.$inject = ['$scope', '$http', 'liquidarConceptosService', '$window', 'growl', 'ValidationService', 'appConstant', 'appGenericConstant', 'utilServices', 'localStorageService'];

    function liquidarConceptosCtrl($scope, $http, liquidarConceptosService, $window, growl, ValidationService, appConstant, appGenericConstant, utilServices, localStorageService) {

        var liquidarConceptosControl = this;
        liquidarConceptosControl.nuevoLiquidarConceptos = liquidarConceptosService.liquidarConceptos;
        liquidarConceptosControl.liquidarConceptosAux = liquidarConceptosService.liquidarConceptosAuxiliar;
        liquidarConceptosControl.liquidarConceptosAuxiTotal = liquidarConceptosService.liquidarConceptosAuxiTotal;
        liquidarConceptosControl.datosEstudianteSer = liquidarConceptosService.datosEstudianteSer;
        ...
        function onConsultarConceptosFacturacion() {
            liquidarConceptosService.consultarConceptosALiquidar().then(function (data) {
        ...
        liquidarConceptosControl.onConsultarEstudiante = function () {
            ...
            var codEstudiante = liquidarConceptosControl.nuevoLiquidarConceptos.codigoEstudianteCampo;
            if (new ValidationService().checkFormValidity($scope.formConsultarEstudiante)) {
                ...
                liquidarConceptosService.consultarEstudiante(codEstudiante, 'LiqConceptosCont').then(function (data) {
                    switch (data.tipo) {
                        case 200:
        ...
        liquidarConceptosControl.onCambiarConcepto = function () {
            ...
            if (liquidarConceptosControl.concepFacturacion.conceptoAFacturar !== null && liquidarConceptosControl.concepFacturacion.conceptoAFacturar.codigo !== 'CFI10') {
                ...
                liquidarConceptosService.consultarDetalleConceptoALiquidar(objFacturar).then(function (data) {
                    switch (data.tipo) {
                        case 200:
        ...
        liquidarConceptosControl.onGuardar = function () {
            ...
            liquidarConceptosService.guardarLiquidacion(liquidacionObj).then(function (data) {
                appConstant.CERRAR_SWAL();
                switch (data.tipo) {
                    case 200:
