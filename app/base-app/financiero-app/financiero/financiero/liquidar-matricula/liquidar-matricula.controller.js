(function () {
    'use strict';
    angular.module('mytodoApp').controller('liquidarMatriculaCtrl', liquidarMatriculaCtrl);

    liquidarMatriculaCtrl.$inject = ['$scope', '$http', 'liquidarConceptosService', 'liquidarMatriculaService', 'ValidationService', 'appConstant', 'appGenericConstant', 'utilServices', 'localStorageService'];
    function liquidarMatriculaCtrl($scope, $http, liquidarConceptosService, liquidarMatriculaService, ValidationService, appConstant, appGenericConstant, utilServices, localStorageService) {
        ...
        liquidarMatriculaControl.onConsultarEstudiante = function () {
            ...
            if (new ValidationService().checkFormValidity($scope.formConsultarEstudiante)) {
                ...
                liquidarMatriculaService.consultarEstudiante(codEstudiante, 'LiqMatCont').then(function (data) {
                    switch (data.tipo) {
                        case 200:
                            appConstant.CERRAR_SWAL();
