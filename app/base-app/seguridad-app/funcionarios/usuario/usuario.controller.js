(function () {
    'use strict';
    angular.module('mytodoApp').controller('UsuarioCtrl', UsuarioCtrl);
    UsuarioCtrl.$inject = ['$scope', '$location', 'usuarioEntitiesServices', 'turnoService', 'ValidationService', 'localStorageService', '$http', 'appConstant', 'utilServices', 'appGenericConstant'];
    function UsuarioCtrl($scope, $location, usuarioEntitiesServices, turnoService, ValidationService, localStorageService, $http, appConstant, utilServices, appGenericConstant) {
        var usuarioControl = this;
        usuarioControl.usuarioEntity = usuarioEntitiesServices.usuario;
        usuarioControl.usuarioVisor = usuarioEntitiesServices.usuarioAux;
        usuarioControl.visible = usuarioEntitiesServices.visible;
        ...
        usuarioControl.seccionales = [];

        if (localStorageService.get('usuario') !== null) {
            usuarioControl.usuario = localStorageService.get('usuario');
        }

        if (localStorageService.get('turno') !== "" && localStorageService.get('turno') !== undefined && localStorageService.get('turno') !== null) {
            if(localStorageService.get('turno')[0] !== undefined){
                usuarioControl.usuarioTurno = localStorageService.get('turno')[0];
            }
        ...
        function ejecutarConsultarSeccional() {
            utilServices.getSeccional().then(function (data) {
                usuarioControl.seccionales = [];
                usuarioControl.seccionales = data;
            }).catch(function (e) {
                return;
            });
        };
        ...
        function onConsultarCargos() {
            usuarioEntitiesServices.cargarListaCargo().then(function (data) {
                usuarioControl.usuarioEntity.listaCargo = [];
                usuarioControl.usuarioEntity.listaCargo = data;
            });
        }

        usuarioControl.onGestionarContrato = function (item) {
            usuarioControl.usuarioEntityContrato = {};
            usuarioControl.usuarioEntityContrato.id = item.id;
            $('#modalGestionarContrato').modal({backdrop: 'static', keyboard: false});
            $("#modalGestionarContrato").modal("show");
        };

        usuarioControl.onListarContratos = function (item) {
            usuarioControl.listContratos = [];
            usuarioControl.nombreCompleto = "";
            usuarioControl.nombreCompleto = item.nombreCompleto;
            usuarioEntitiesServices.onListaContratosByIdFuncionario(item.id).then(function (data) {
                usuarioControl.listContratos = data;
                $('#modalListarContratos').modal({backdrop: 'static', keyboard: false});
                $("#modalListarContratos").modal("show");
            });
        };

        usuarioControl.userGestionTurno = {};
        usuarioControl.onAsignarTurnos = function (item) {
            usuarioControl.nombreCompleto = item.nombreCompleto;
            usuarioControl.userGestionTurno = item;
            $('#modalAsignarTurno').modal({backdrop: 'static', keyboard: false});
            $("#modalAsignarTurno").modal("show");
        };

        usuarioControl.onGuardarModuloTurno = function () {
            usuarioControl.usuarioTurno.activo = "1";
            usuarioControl.usuarioTurno.idUsuario = usuarioControl.userGestionTurno.id;
            
            turnoService.onPostAgregarTurno(usuarioControl.usuarioTurno).then(function (data) {
                $("#modalAsignarTurno").modal("hide");
                swal({
                    text: "EL USUARIO MODIFICADO, DEBE CERRAR SESIÓN Y VOLVER A INICIAR PARA QUE LOS CAMBIOS SEAN TOMADOS",
                    type: "success",
        ...
        usuarioControl.guardarContrato = function () {
            usuarioControl.usuarioEntityContrato.fechaInicio = appConstant.TO_DATE_LONG(usuarioControl.usuarioEntityContrato.fechaInicio);
            usuarioControl.usuarioEntityContrato.fechaFin = appConstant.TO_DATE_LONG(usuarioControl.usuarioEntityContrato.fechaFin);
            usuarioControl.usuarioEntityContrato.idUsuarioRegistra = localStorageService.get('usuario').id;
            usuarioControl.usuarioEntityContrato.idCargo = usuarioControl.usuarioEntityContrato.cargoE.id;
            usuarioControl.usuarioEntityContrato.cargo = usuarioControl.usuarioEntityContrato.cargoE.nombreCargo;
            usuarioEntitiesServices.onAgregarContrato(usuarioControl.usuarioEntityContrato).then(function (data) {
                $("#modalGestionarContrato").modal("hide");
                appConstant.CERRAR_SWAL();
                appConstant.MSG_GROWL_OK(appGenericConstant.REGISTRO_GUARDADO);
        ...
        function onBuscarUsuario() {
            ...
            usuarioEntitiesServices.buscarUsuario().then(function (data) {
                usuarioControl.listaUsuario = [];
                var usuarios = [];
                angular.forEach(data, function (value, key) {
                    usuarios = {
                        apellidoFuncionario: value.apellidoFuncionario,
                        nombreFuncionario: value.nombreFuncionario,
                        tipoIdentificacion: value.identificacionFuncionario.idTipoIdentificacion,
                        identificacion: value.identificacionFuncionario.identificacion,
                        nombreCompleto: (value.nombreFuncionario + " " + value.apellidoFuncionario),
                        identificacionConTipo: (value.identificacionFuncionario.nombreTipoIdentificacion + " " + value.identificacionFuncionario.identificacion),
                        estado: value.estado,
                        id: value.id,
                        ...
                        usuarioSistema: value.usuario.userName,
                        sede: value.usuario.idUniversidad
                    };
        ...
        usuarioControl.uploadPhoto = function (file) {
            if (file.name !== appGenericConstant.INDEFINIDO) {
                ...
                var urlRequest = usuarioEntitiesServices.subirarchivos();
                var fd = new FormData();
                fd.append('file', file);
        ...
        usuarioControl.downloadFoto = function (item) {
            try {
                if (item.idDocumento !== null || item.idDocumento !== appGenericConstant.INDEFINIDO) {
                    var file = usuarioEntitiesServices.downloadFoto(item);
        ...
        usuarioControl.onLimpiarRegistro = function () {
            ...
            usuarioControl.usuarioEntity.sede = null;
                        ...
                        var newUsuario =
                                {
                                    ...
                                    nombreFuncionario: onValidarTipoString(usuarioControl.usuarioEntity.nombre),
                                    apellidoFuncionario: onValidarTipoString(usuarioControl.usuarioEntity.apellido),
                                    ...
                                    usuario: {
                                        ...
                                        idUniversidad: usuarioControl.usuarioEntity.sede
                                    }

                                };
                        if (usuarioControl.visible.validocelsize === false || usuarioControl.visible.validotelefonosize === false) {
                            usuarioEntitiesServices.agregarUsuario(newUsuario).then(function (response) {
        ...
        usuarioControl.editarUsuario = function (usuarioControl) {
            var updateUsuario =
                    {
                        ...
                        idGenero: usuarioControl.usuarioEntity.idGenero,
                        idEstadoCivil: usuarioControl.usuarioEntity.idEstadoCivil,
                        nombreFuncionario: onValidarTipoString(usuarioControl.usuarioEntity.nombre), apellidoFuncionario: onValidarTipoString(usuarioControl.usuarioEntity.apellido),
                        fechaNacimiento: toDate(usuarioControl.usuarioEntity.fechaNacimiento),
            ...
            if (usuarioControl.visible.validocelsize === false || usuarioControl.visible.validotelefonosize === false) {
                usuarioEntitiesServices.actualizarUsuario(updateUsuario).then(function (data) {
        ...
        usuarioControl.onClickToEditar = function (item) {
            ...
            usuarioControl.usuarioEntity.sede = item.sede;
        ...
        usuarioControl.onClickToDelete = function (item) {
            ...
            swal({
                ...
                if (isConfirm) {
                    usuarioEntitiesServices.eliminarUsuario(item.id).then(function (response) {
                        switch (response.tipo) {
                            case 200:
        ...
        usuarioControl.onClickToDeleteMasivo = function () {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    usuarioEntitiesServices.eliminarUsuarioMasivo(listaElementosEliminar).then(function (response) {
                        switch (response.tipo) {
                            case 200:
