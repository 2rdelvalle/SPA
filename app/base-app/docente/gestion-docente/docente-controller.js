(function () {
    'use strict';
    angular.module('mytodoApp').controller('DocenteCtrl', DocenteCtrl);
    DocenteCtrl.$inject = ['$scope', '$location', 'docenteEntitiesServices', 'growl', 'ValidationService', 'localStorageService', 'utilServices', '$http', '$window', 'appConstant', 'appGenericConstant'];
    function DocenteCtrl($scope, $location, docenteEntitiesServices, growl, ValidationService, localStorageService, utilServices, $http, $window, appConstant, appGenericConstant) {
        var docenteControl = this;
        docenteControl.docenteEntity = docenteEntitiesServices.docente;
        docenteControl.docenteVisor = docenteEntitiesServices.docenteAux;
        docenteControl.visible = docenteEntitiesServices.visible;
        ...
        docenteControl.seccionales = [];
        docenteControl.timestamp = new Date().getTime();
        var fecha;

      $scope.$watch('DOCtrl.docenteEntity.foto', function(newValue, oldValue) {
        ...
        function consultarPaises() {
            docenteEntitiesServices.consultarPais().then(function (data) {
        ...
        function onBuscarDocente() {
            docenteEntitiesServices.buscarDocente().then(function (data) {
        ...
        docenteControl.onConsultarDepartoPorPais = function (item) {
            ...
            docenteEntitiesServices.consultarDepartamentoPais(item).then(function (data) {
                docenteControl.docenteEntity.lgrecidencialstdepartamentos = [];
        ...
        docenteControl.onConsultarMunicipioPorDepartamento = function (item) {
            ...
            docenteControl.visible.activomsjpaislgrecidencia = false;
            docenteEntitiesServices.consultarMunicipioPorDepartamento(item).then(function (data) {
                docenteControl.docenteEntity.lgrecidencialstmunicipios = [];
        ...
        docenteControl.onLugarResidencia = function (item) {
            ...
            docenteEntitiesServices.consultarBarrios(docenteControl.docenteEntity.idMunicipioRecidencia).then(function (data) {
        ...
        docenteControl.uploadPhoto = function (file) {
          ...
          var fd = new FormData();
          fd.append('file', file);
          fd.append('student', false);

          fd.append('documento', docenteControl.docenteEntity.identificacion);
          ...
          docenteEntitiesServices.getFuncionario(docenteControl.docenteEntity.identificacion).then(function (info) {
            setTimeout(function() {
            window.location.reload();
          }, 5000);
            docenteControl.docenteEntity.foto = info.idFoto;
          });
        };
        ...
        docenteControl.uploadSoporteAcademico = function (file) {
            docenteControl.item = [];
            var urlRequest = docenteEntitiesServices.subirSoporte();
            var fd = new FormData();
            fd.append('file', file);
        ...
        docenteControl.uploadSoporteHv = function (file) {
            ...
            var urlRequest = docenteEntitiesServices.subirHvDocente();
            var fd = new FormData();
            fd.append('file', file);
            $http.post(urlRequest, fd, {
                transformRequest: angular.identity,
        ...
        docenteControl.downloadSoporteHv = function (item) {
            if (item.id !== null || item.id !== undefined) {
                var file = docenteEntitiesServices.descargaHv(item.id);
        ...
        docenteControl.downloadSoporteAcademico = function (item) {
            if (item.idDocumento !== null || item.idDocumento !== undefined) {
                var file = docenteEntitiesServices.downloadArchivo(item.idDocumento);
                if (file !== null && item !== null && item !== undefined) {
                    $window.location.href = file;
                }
            }
        ...
        docenteControl.downloadFoto = function (item) {
            if (item !== null || item !== undefined) {
                var file = docenteEntitiesServices.downloadFoto(item);
                if (file !== null && item !== null && item !== undefined) {
        ...
        docenteControl.onLimpiarRegistro = function () {
            ...
            docenteControl.docenteEntity.idSeccional = null;
        ...
        docenteControl.onNewRegistryDocente = function () {
            if (typeof docenteControl.docenteEntity.idMunicipioRecidenciaNacimiento === undefined || docenteControl.docenteEntity.idMunicipioRecidenciaNacimiento === null) {
                ...
                if (docenteControl.docenteEntity.id === null || typeof docenteControl.docenteEntity.id === undefined) {
                    var newDocente =
                            {
                                ...
                                idSeccional: docenteControl.docenteEntity.idSeccional
                            };
                    if (typeof docenteControl.docenteEntity.listaIformacionAcademica !== undefined && docenteControl.docenteEntity.listaIformacionAcademica !== null
                            && docenteControl.docenteEntity.listaIformacionAcademica.length !== appGenericConstant.CERO) {
                        docenteEntitiesServices.agregarDocente(newDocente).then(function (response) {
                    ...
                    docenteEntitiesServices.actualizarDocente(updateDocente).then(function (data) {
                        if (data.tipo === 200) {
        ...
        docenteControl.onClickToView = function (item) {
          ...
          docenteEntitiesServices.getFuncionario(docenteControl.docenteEntity.identificacion).then(function (info) {
            ...
            docenteControl.docenteEntity.idSeccional= item.idSeccional;
        ...
        docenteControl.onClickToEditar = function (item) {
            ...
            docenteEntitiesServices.consultarDepartamentoPais(item.idPais).then(function (data) {
                docenteControl.docenteEntity.lgrecidencialstdepartamentos = [];
                docenteControl.docenteEntity.lgrecidencialstdepartamentos = data;
                docenteEntitiesServices.consultarMunicipioPorDepartamento(item.departamento).then(function (data) {
                    docenteControl.docenteEntity.lgrecidencialstmunicipios = [];
                    docenteControl.docenteEntity.lgrecidencialstmunicipios = data;
                    docenteEntitiesServices.consultarDepartamentoPais(item.PaisNacimiento).then(function (data) {
                        docenteControl.docenteEntity.lgrecidencialstdepartamentosNacimiento = [];
                        docenteControl.docenteEntity.lgrecidencialstdepartamentosNacimiento = data;
                        docenteEntitiesServices.consultarMunicipioPorDepartamento(item.departamentoNacimiento).then(function (data) {
                            docenteControl.docenteEntity.lgrecidencialstmunicipiosNacimiento = [];
                            docenteControl.docenteEntity.lgrecidencialstmunicipiosNacimiento = data;
        ...
        docenteControl.onClickToDelete = function (item) {
            ...
            swal({
                ...
                if (isConfirm) {
                    docenteEntitiesServices.eliminarDocente(item.id).then(function (response) {
                        switch (response.tipo) {
                            case 200:
        ...
        docenteControl.onClickToDeleteMasivo = function () {
            ...
            swal({
                ...
                if (isConfirm) {
                    ...
                    docenteEntitiesServices.eliminarDocenteMasivo(listaElementosEliminar).then(function (response) {
                        switch (response.tipo) {
                            case 200:
        ...
        function ejecutarConsultarSeccional() {
            docenteEntitiesServices.consultarSeccional().then(function (data) {
                docenteControl.seccionales = [];
                docenteControl.seccionales = data;
            }).catch(function (e) {
                return;
            });
        };
        ...
        function findSede() {
            docenteEntitiesServices.getSedes().then(function (data) {
                docenteControl.sedes = data;
            }).catch(function (e) {
                return;
            });
        };
